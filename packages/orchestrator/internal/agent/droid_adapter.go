...
func (a *DroidAdapter) BuildRequest(ctx context.Context, req *AgentRequest) (*ExecutorRequest, error) {
	...
	// Authentication: BYOK mode vs Factory platform mode
	if a.providerType != "" && a.baseURL != "" && a.apiKey != "" {
		// BYOK mode: configure custom model via settings.json (generated by entrypoint.sh)
		env["DROID_PROVIDER_TYPE"] = a.providerType
		env["DROID_BASE_URL"] = a.baseURL
		env["DROID_API_KEY"] = a.apiKey
	} else {
		// Factory platform mode: use FACTORY_API_KEY
		factoryKey := os.Getenv("FACTORY_API_KEY")
		if factoryKey != "" {
			env["FACTORY_API_KEY"] = factoryKey
		}
	}
